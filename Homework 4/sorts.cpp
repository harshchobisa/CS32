...
void insertion_sort(vector<Store>& s, bool comp(const Store&, const Store&))
{
    ...
    // final ordering.
    for (int items = 1; items < s.size(); items++)
    {
        Store nextItem = s[items];
        int current = items;
        while ((current > 0) && comp(nextItem, s[current - 1]))
        {
